	...
	<?php	
		...
		?>
					<tr class="cart_products_tr">
						...
						<td align="center">Rs. <?=$line_cost?></td>
                        <td align="center"><a href="<?=$_SERVER['PHP_SELF']?>?action=remove&id=<?=$product_id?>" onclick="return confirm('Are you sure want to Remove?');"><img width="18px" height="18px" src="images/2.png" title="Delete Product"></a></td>
					</tr>
		<?php 
				}
			} 
		...
		?>
			...
			<tr class="cart_pro_total_tr">
				...
				<td colspan="2" align="right">Rs. <?=$total?></td>
			</tr>
            
             <?php $total_usd=currency("INR","USD",$total); $total_usd=round($total_usd, 2)?>
			...
			<tr bgcolor="#F3E4B9">
                <td colspan="6" align="right">-->
                 ...
                 <?php
				$adminurl=mysql_query(mysql_query("select url from admin_master where admin_id=1"));
                ...
                <?php /*if($_SESSION['user_name']!=''){$url=$paypal_url ;} else{ $url='single_login.php';}*/?> <!------- paypal url------>
                    ...
                    <form action="<?=$url?>" method="post" name="paypal" >
                        <input name="amount" type="hidden" id="amount"   value="<?=$total_usd?>">
                        <input type="hidden" name="cmd" value="_xclick">
                       <!-- <input type="hidden" name="business" value="bibhas.m@arhamcreation.com">-->
                       <input type="hidden" name="business" value="jnj.vijay@yahoo.co.in">
                        <input type="hidden" name="item_name" value="Items">
                        <input type="hidden" name="return" value="<?=$admin_url?>payment_success.php" />
