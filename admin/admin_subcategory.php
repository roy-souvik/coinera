...
function main()
{
	...
	$sql="SELECT count(*) FROM category_info where p_id=".$category_id;
	$res=mysql_query($sql);
	$row = mysql_fetch_row($res);
    $count =  $row[0];
	
	$color_sql="SELECT *  FROM category_info where p_id=".$category_id." ORDER BY id asc LIMIT $GLOBALS[start],$GLOBALS[show]";
	$row=mysql_fetch_array(mysql_query("select count(*) FROM category_info where p_id=".$category_id." ORDER BY id asc"));
	$count=$row[0];
...
<table width="99%" align="center" border="0" cellpadding="5" cellspacing="1">
		...
		<tr> 
			<td width="99%" align="center" class="ErrorText"><?=$GLOBALS['err_msg']?> </td>
            
    <td width="7%" align="right"><a href="javascript:window.location='admin_category.php';" title=" Refresh the page"><img border="0" src="images/back.png"></a></td>
...
<table width="99%" align="center" border="0" cellpadding="5" cellspacing="2"  class="ThinTable">
  ...
  <tr class="TDHEAD"> 
            ...
            <?php $sql_sct=mysql_fetch_array(mysql_query("select name from category_info where id=".$category_id)); ?> 
              Sub Category List of <?=$sql_sct['name']?></td>
          </tr>
  <? 
	if($count == 0)
	{ 
  ...
  <tr class="text_normal"> 
    ...
    <td width="14%" bgcolor="#CFC5BC"><strong>Sub Category</strong></td>
  ...
  <tr>
    <td align="center"><table width="100%" align="center"  cellpadding="4" cellspacing="4" >
          <tr class="TDHEAD"> 
            ...
            <?php $sql_sct1=mysql_fetch_array(mysql_query("select name from category_info where id=".$_POST['c_id'])); ?>
              <?=$current_mode?>
              Sub Category of <?=$sql_sct1['name']?></td>
          </tr>
          <tr> 
          ...
          <tr> 
            ...
            <td align="left"><input type="text" name="title" style="width:200px;"  value="<?=$rec['name']?>"></td>
          </tr>
          ...
          <tr> 
            ...
            <td class="point_txt">
            <?php
			$id_c=base64_encode($_POST['c_id']);
			
			?>
            <input name="submit" type="submit" class="inplogin" value="<?=$_REQUEST['mode']=='add'?'Add':'Update'?>"> 
              &nbsp; 
                 <input name="button" type="button" class="inplogin" onClick="javascript:window.location='admin_subcategory.php?id=<?=$id_c?>';"  value="Cancel"> 
            </td>
          </tr>
        </table></td>
  </tr>
...
function delete_record($id)
{
 ...
 {  
 	...
 	if($row_id=='') {
			echo $sql="INSERT INTO category_info
				  SET 
				  name = '".$_POST['title']."',
				  date=CURDATE(),
				  p_id=".$_POST['c_id'];
		...
		$sql_update="UPDATE category_info   
						 SET  
						 name = '".$_POST['title']."'
				  		 WHERE id = ".$row_id;
...
function active_deactive($id)
{
	...
	$sql = "UPDATE category_info SET status = if(status = 'N','Y','N') WHERE id = '$id'";
	mysql_query($sql);
