...
function main()
{
	...
	$sql="SELECT count(*) FROM category_info where p_id=0";
	$res=mysql_query($sql);
	$row = mysql_fetch_row($res);
    $count =  $row[0];
	
	$color_sql="SELECT *  FROM category_info where p_id=0 ORDER BY id asc LIMIT $GLOBALS[start],$GLOBALS[show]";
	$row=mysql_fetch_array(mysql_query("select count(*) FROM category_info where p_id=0 ORDER BY id asc"));
	$count=$row[0];
...
function cate(id) {
	//alert(id);
	  ...
	  xmlhttp.open("GET","admin_subcategory.php?id="+id,true);
	  xmlhttp.send();
	} 
</script>
...
<table width="99%" align="center" border="0" cellpadding="5" cellspacing="2"  class="ThinTable">
  ...
  <tr onMouseOver="this.bgColor='<?=SCROLL_COLOR;?>'" onMouseOut="this.bgColor=''" class="text_small"> 
    ...
    <td valign="top"><a href="admin_subcategory.php?id=<?=base64_encode($rec['id'])?>" title="Add Sub Category"><?=$rec['name']?></a></td>
