...
<div class="pdtt_glry_select">
	Sort By : <select name="product_type" class="product_type">
	...
	<script type="text/javascript" src="scripts/product_type.js"></script>
</div>
<br clear="all">
<style>
.border_select{
  ...
  if($row > 0){
	while($product=mysql_fetch_array($sql)){
		...
		if($product['bid_price']!=0 && $product['direct_price']==0 ){
			...
			?>
		<div class="outerdiv" style="margin-top:10px;">
   ...
   <?php
    ...
    <?php
        ...
        <span class="pgbtn"><a class="page" name="<?=$prd_id?>" value="1">First</a></span>
        
        <span class="pgbtn"><a class="page" name="<?=$prd_id?>" value="<?=$page_multiplier?>">Prev</a></span>
        <?php } ?>
        ...
        <?php } else { ?>
        <span class="pgbtn"><a class="page" name="<?=$prd_id?>" value="<?=$page_no?>"><?=$page_no?></a></span>
        <?php } ?>
        <?php } ?>
        ...
        <span class="pgbtn"><a class="page" name="<?=$prd_id?>" value="<?=$page_multiplier+2?>">Next</a></span>
        <span class="pgbtn"><a class="page" name="<?=$prd_id?>" value="<?=ceil($post_nos/$post_per_page)?>">Last</a></span>
